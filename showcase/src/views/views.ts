...
export function TokensView(tokens: {
  ...
  const rows = tokens.vars
    .map(
      (t) =>
        `<tr><td><code>${esc(t.name)}</code></td><td><code>${esc(t.light)}</code></td><td><span class="swatch" style="background:${esc(t.hex || t.light)};background:${esc(t.light)}" role="img" aria-label="Muestra ${esc(t.name)}"></span></td><td><code>${esc(t.hex)}</code></td><td><code>${esc(t.dark)}</code></td></tr>`
    )
    .join("");
...
export function ComponentesView(): string {
  const cards = registry
    .map(
      (c) =>
        `<article class="card" data-component="${esc(c.id)}"><p><span class="badge">${esc(c.category)}</span> <code>${esc(c.id)}</code></p><h2>${esc(c.name)}</h2><p class="caption">${esc(c.description)}</p><div class="preview">${c.render(c.sample as never)}</div></article>`
    )
    .join("");
...
export function PreviewView(
  ...
  const tabs = pages
    .map(
      (p) =>
        `<a class="btn" data-variant="${p.slug === active ? "primary" : "secondary"}" href="#/preview/${esc(p.slug)}">${esc(p.slug)}</a>`
    )
    .join("");
  ...
  const options = invalidCodes
    ...
    .join("");
  return `<div class="stack"><h1>Preview del bundle golden</h1><p class="lead">Render de <code>fixtures/golden/clinica-dental-sur</code> con los componentes reales.</p><div class="cluster" role="tablist" aria-label="Páginas">${tabs}</div><article class="card"><div class="stack">${current ? current.html : renderByType("text-block", { content: "Sin página", variant: "body" })}</div></article><h2>Fixtures inválidas</h2><p class="caption">Selector informativo: el validador CLI emite el código esperado. La GUI no rompe, muestra el aviso.</p><form class="cluster" id="invalid-picker"><label for="invalid-code">Código</label><select id="invalid-code" name="code">${options}</select><output id="invalid-out" aria-live="polite">${invalidCode ? `El fixture <code>${esc(invalidCode)}</code> dispara <code>[${esc(invalidCode)}]</code> en <code>webconfig validate</code>.` : "Elige un código para ver el mensaje esperado."}</output></form></div>`;
}
